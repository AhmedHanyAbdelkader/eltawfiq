import 'package:dartz/dartz.dart';
import 'package:eltawfiq_suppliers/core/error/failure.dart';
import 'package:eltawfiq_suppliers/core/use_case/base_use_case.dart';
import 'package:eltawfiq_suppliers/suppliers/domain/entities/group_entity.dart';
import 'package:eltawfiq_suppliers/suppliers/domain/repository/suppliers_base_repository.dart';

class GroupsUseCase extends BaseUseCase<List<GroupEntity>, NoParameters>{
  SuppliersBaseRepository suppliersBaseRepository;
  GroupsUseCase(this.suppliersBaseRepository);

  @override
  Future<Either<Failure, List<GroupEntity>>> call(NoParameters parameter) async{
    return await suppliersBaseRepository.getGroups(parameter);
  }

}