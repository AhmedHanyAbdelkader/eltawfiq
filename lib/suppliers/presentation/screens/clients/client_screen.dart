...
class ClientScreen extends StatefulWidget {
  const ClientScreen({super.key, required this.supplier});
...
class _ClientScreenState extends State<ClientScreen> {
  ...
  _fetchData() async {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<GetClientController>(context, listen: false).getClient(
          widget.supplier.id!);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<GetClientController>(
        builder: (context, getSupplierController, _) {
          if (getSupplierController.getClientIsLoading) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          }
          if (getSupplierController.getClientErrorMessage.isNotEmpty) {
            return Center(
              child: Text(getSupplierController.getClientErrorMessage),
            );
          }
          if (getSupplierController.gettingClient == null) {
            return const Center(
              child: Text('No Client available'),
            );
          }
          else {
            final SupplierEntity? supplier = getSupplierController.gettingClient;
