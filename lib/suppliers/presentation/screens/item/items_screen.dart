...
class _ItemsScreenState extends State<ItemsScreen> {
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text(StringManager.items),
        ...
        body: Consumer<ItemsController>(
          builder: (context, itemsController, _) {
            if (itemsController.getItemsIsLoading) {
              ...
              return ItemsListView(items: itemsController.gettingItems, role: role!,);
              //   ListView.separated(
              ...
              //       onTap: (){
              //         RouteGenerator.navigationTo(AppRoutes.itemOperationsScreen, arguments: item);
              //       },
              //       onLongPress: () async {
              //         showDialog(
              //           context: context,
...
class EditItemDialog extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    final TextEditingController dastaController = TextEditingController(text: item.dasta.toString());
    final TextEditingController kartonaController = TextEditingController(text: item.kartona.toString());
    final TextEditingController balanceKetaaController = TextEditingController(text: item.balanceKetaa.toString());
    final TextEditingController balanceDastaController = TextEditingController(text: item.balanceDasta.toString());
    final TextEditingController balanceKartonaController = TextEditingController(text: item.balanceKartona.toString());
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'الكرتونه تحتوي علي كام دسته'),
                //validator: (value) => value!.isEmpty ? 'Please enter item code' : null,
                keyboardType: TextInputType.number,
              ),
              TextFormField(
                controller: dastaController,
                decoration: const InputDecoration(labelText: 'الدسته تحتوي علي كام قطعه'),
                //validator: (value) => value!.isEmpty ? 'Please enter item code' : null,
                keyboardType: TextInputType.number,
              ),
            ...
            if (formKey.currentState!.validate()) {
              ...
              await editItemController.editItems(
                EditItemParameters(
                  ...
                  balanceKartona: balanceKartonaController.text.isNotEmpty
                      ? int.tryParse(balanceKartonaController.text)
                      : item.balanceKartona,
                  balanceDasta: balanceDastaController.text.isNotEmpty
                      ? int.tryParse(balanceDastaController.text)
                      : item.balanceDasta,
                  balanceKetaa: balanceKetaaController.text.isNotEmpty
                      ? int.tryParse(balanceKetaaController.text)
                      : item.balanceKetaa,
                ),
              );
              Provider.of<ItemsController>(context, listen: false).getItems(const NoParameters());
